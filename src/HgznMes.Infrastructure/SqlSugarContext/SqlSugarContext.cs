...
namespace HgznMes.Infrastructure.SqlSugarContext;
...
public sealed class SqlSugarContext
{
    ...
    private ConnectionConfig Build()
    {
        ...
        var connConfig = new ConnectionConfig()
        {
            ...
            ConfigureExternalServices = new ConfigureExternalServices()
            {
                ...
                {
                    //配置表外键
                    c.IfTable<EquipLedgerAggregateRoot>()
                        .OneToOne(t => t.Room, nameof(EquipLedgerAggregateRoot.RoomId));
    ...
    private void InitTables()
    {
        ...
        // {
        //     var tableName = table.FullName?.Split('.').Last();
        //     var tableAttr = table.GetCustomAttribute<TableAttribute>();
        //     if (tableAttr != null)
        //     {
        //         tableName = tableAttr.Name;
        //     }
        //     var tableDescription = table.GetCustomAttribute<DescriptionAttribute>();
        //     
        //     DbContext.CodeFirst.InitTables(table);
        // }
    ...
    private void OnLogExecuted(string sql, SugarParameter[] pars)
    {
        if (DbOptions.EnabledSqlLog)
        {
            var sqllog = $"=========Yi-SQL耗时{DbContext.Ado.SqlExecutionTime.TotalMilliseconds}毫秒=====";
