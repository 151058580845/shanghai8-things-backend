...
namespace HgznMes.Application.Captchas
{
    public static class CaptchaUtil
    {
            ...
            bool noise = false, bool line = false, bool circle = false)
        {
            ...
            using var typeface = SKTypeface.FromFamilyName(options.FontFamily, SKFontStyleWeight.SemiBold, SKFontStyleWidth.ExtraCondensed, SKFontStyleSlant.Upright);
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                drawStyle.Color = Colors[random.Next(0, Colors.Length - 1)];
                canvas.DrawText(text[i].ToString(), px, py, SKTextAlign.Center, font, drawStyle);
                canvas.RotateDegrees(-angle, px, py);
                canvas.Translate(-offset, -offsetY);
            }
